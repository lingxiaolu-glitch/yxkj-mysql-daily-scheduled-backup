...
> **当前进度（部分完成）**：已完成数据库实测：本机 MySQL 5.7.39 隔离实例 L1/L2/恢复演练，以及服务器 MySQL 8.0.32（117.72.97.228:13306）真实完整备份 + schema_only=true 验证，结果见 `docx/DRILL_REPORT_LATEST.md`；生产 Linux 部署仍需 SSH 权限、实例 B 真实信息和磁盘实测，不能标记完成。
